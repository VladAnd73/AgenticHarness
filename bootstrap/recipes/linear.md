...
- `LINEAR_API_KEY` -- a Linear **personal API key** from
...
All assume `spore-with-secrets` is on PATH and `LINEAR_API_KEY`
resolves.
...
- Never echo `$LINEAR_API_KEY` to a pane or log. Shape-check with
